...
Serial puerto;
int bloque1, bloque2, bloque3, bloque4, bloque5, bloque6, D1, D2, D3, data, acele, temp, analog1, analog2, analog3, prev, level, x, hora, minut;
int add,t,T,promtemp,promlevel,promlevelant,promedio;
PImage termocold, termohot, tank, tempcold, temphot, moist, dry, sun, cloud, rain, norain, sprinkler;
...
void setup(){
  ...
  sprinkler = loadImage("sprinkler.png");
  image(termocold, 0, 0);
  image(tank, 400, 0);
  cuadricula();
...
void draw(){
    ...
    if(puerto.read() == 243){            // Verifico que se haya leido el header
      ...
      D3 = (bloque3>>6)&1;               // Sensor de lluvia
     
      // Se notifica el estado de los sensores digitales
      ...
      hora = hour();
      minut = minute();
      if (hora==15 && minut>=40 && minut<=50) image(sprinkler, 720, 765);    // Hora en la que se riega el cultivo (De 3:40pm a 3:50pm)
      ...
      if(t!=100) promtemp = promtemp + temp;                 // Se toman 100 muestras de temperatura para hacer un promedio
        ...
        t=0;
        printerm(promtemp);                                  // Se utiliza la funcion printerm para graficar un termometro que muestra la temperatura medida
      ...
      level = int(map(analog3, 0, 250, -300, 0));
      if(T!=100) promlevel = promlevel + level;              // Se toman 100 muestras de distancia para hacer un promedio
      else{
        promedio = promlevel/100;
        T=0;
        promlevel=0;
        if (abs(promlevelant-promedio)<5){                  // Filtro digital del sensor ultrasonico
          if(promedio<0) printank(promedio);                // Se utiliza la funcion printank para graficar un tanque que muestra el nivel de agua
          printbubble();
        }
      ...
      if (promedio<=-270){
        fill(26, 55, 157);
        textSize(16);
        text("El nivel del tanque es óptimo", 480, 20);   
      }      
      }
   }
}
...
void printerm(int temp){
    ...
    fill(1, 144, 233);
    stroke(1, 144, 233);
    rect(175, 416, 35, temp-15);  // X,Y,Ancho,Alto
    fill(255);
    textSize(35);
    text(+(-temp/8), 300, 525); 
    text("°C", 350, 525); 
